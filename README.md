...
搜索引擎 & 智能体工程师，用 **Go / Python / C++** 构建 AI 基础设施。

- 🔭 项目方向：LLM 网关 · 工作流引擎 · 向量数据库 · 搜索引擎 · 网络爬虫
- 🧰 技术栈：Go · Python · C++ · 分布式系统 · LLM 集成 · 基础设施
...
| 项目 | 简介 |
| :--- | :--- |
| **[baize](https://github.com/wzhongyou/baize)** | Baize（白泽）— 通用超级智能体，问答、编程、创作、办公全覆盖。CLI / TUI / IDE / Web / IM Bot 多入口，单二进制分发，对标 Manus / Codex 等前沿产品。 |
| **[suwen](https://github.com/wzhongyou/suwen)** | Suwen（素问）— 开源 AI 搜索引擎，混合召回 + 多步推理，对标 Perplexity / 秘塔。查询理解 → 混合检索(vortex+proximia) → 排序 → LLM 生成答案+引用，全链路自建，不依赖商业搜索 API。 |
...
| **[llmgate](https://github.com/wzhongyou/llmgate)** | Go 原生 LLM 网关 — 统一 API 对接 20+ 模型供应商，单次转发 <10ms，内置 Web 控制台，支持 function calling、自动故障转移与成本感知路由。 |
